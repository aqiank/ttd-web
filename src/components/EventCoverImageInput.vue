...
<script>
  export default {
    computed: {
      imageURL: {
        get() {
          const imageURL = this.$store.state.eventForm.data.coverImageURL
        ...
        set(value) {
          this.$store.commit('setEventFormCoverImageURL', value)
        },
      }
    },
    methods: {
      onChange(file) {
        ...
        reader.addEventListener('load', (event) => {
          this.$store.commit('setEventFormCoverImageURL', event.target.result)
        })
